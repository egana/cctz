...
bool TimeZoneInfo::ResetToPosixTimeZone(const PosixTimeZone& spec) {
  ...
  transitions_.reserve(400 * 2 + 1);
  transitions_.resize(400 * 2);
  extended_ = true;

  abbreviations_ = spec.std_abbr;
  abbreviations_.append(1, '\0');  // add NUL
  abbreviations_ += spec.dst_abbr;
  abbreviations_.append(1, '\0');  // add NUL
...
bool TimeZoneInfo::Load(const std::string& name) {
  ...
  PosixTimeZone posix;
  if (PosixTimeZoneFromName(name, &posix)) {
    return ResetToPosixTimeZone(posix);
  }
